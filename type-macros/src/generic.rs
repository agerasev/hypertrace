...
use syn::{self, DeriveInput, GenericParam, Data, Fields};
...
pub fn make_where_clause(input: &DeriveInput) -> TokenStream2 {
    let clause = match &input.data {
        Data::Struct(struct_data) => {
            make_where_clause_fields(&struct_data.fields)
        },
        Data::Enum(enum_data) => {
            enum_data.variants.iter().fold(quote!{}, |accum, variant| {
...
pub fn make_params(input: &DeriveInput) -> (TokenStream2, TokenStream2) {
    ...
    let params = input.generics.params.iter().fold(quote!{}, |accum, param| {
